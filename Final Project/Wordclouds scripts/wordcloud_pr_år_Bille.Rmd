...
```{r}
#Follows the guide from software carpentry 
list.files(path = "data:breve_bille", pattern = "brev_bille", full.names = TRUE)
```
```{r}
#Follows the guide from software carpentry 
filenames <- list.files(path = "data:breve_bille",  
                        pattern = "brev_bille-[0-4]{2}.pdf",
                        full.names = TRUE)
for (f in filenames) {
  print(f)
  analyze(f)
...
file_years <- c(
  "brev_bille-01.pdf" = "Letter 1: 1563-10-09",
  "brev_bille-02.pdf" = "Letter 2: 1573",
  "brev_bille-03.pdf" = "Letter 3: 1573-08-15",
  "brev_bille-04.pdf" = "Letter 4: 1574-03-06"
  )
...
all_words_tf_idf %>%
  ...
  labs(
    title = "Beate Bille letters: most unique words per year",
...
```{r}
ggsave("wordcloudBeateBille.png", width = 20, height = 5,  units = "cm", bg = "white")
```
