...
kings1 <- read.csv("data/Kongerækken_clean.csv")
glimpse(kings1)
kings2 <- read_csv("data/Kongerækken_clean.csv")
glimpse(kings2)
kings3 <- read.csv2("data/Kongerækken_clean.csv")
glimpse(kings3)
kings4 <- read_csv2( "data/Kongerækken_clean.csv")
glimpse(kings4)
```
...
You can tell that kings 2 and 4 are tidyverse functions, as they use "_" instead of "."

2. What is the result of running `class()` on the `kings` object created with a tidyverse function.
when running class on kings, it shows "spec_tbl_df" "tbl_df" "tbl" "data.frame", which describes a tibble

3. How many columns does the object have when created with these different functions?
When using function ncol(), there are 6 columns. 

4. Show the dataset so that we can see how R interprets each column

```{r eval=TRUE}
kings <- read_csv2("data/Kongerækken_clean.csv", na="na")
...
```{r}
# I used chatgpt to find the code that removes the missing values
