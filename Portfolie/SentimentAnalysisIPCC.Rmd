---
title: 'Text mining, sentiment analysis, and visualization using Game of Thrones'
author: Cecilie, Sarah, Laura, Amanda
date: '12/5/2025'
...
### Get the GOT pdf:
```{r get-document}
got_path <- "~/BA Historie/Digitale metoder/SentimentAnalysis-main/data/got.pdf"
...
# I added - to get the negative words
...
Taking this script as a point of departure, apply sentiment analysis on the Game of Thrones. You will find a pdf in the data folder. What are the most common meaningful words and what emotions do you expect will dominate this volume? Are there any terms that are similarly ambiguous to the 'confidence' above?
  ...
  Answer: I expect words as "war" and "dragon" to be frequent. I also expect that the sentiment analysis will show, that there are more words associated with negative feelings than positive. 

After running the sentiment analysis, I see that the mean regarding positive or negative words is slightly negative, which matches my expectations. The AFINN graph also shows, that there are more negative than positive words. The three most common negative words are fire, fear and death, with also matches my expectations.

  Answer: The word "lord" is ambiguous, as it applies to four sentiments: disgust, negative, positive and trust. This tells me that they probably adress someone as lord with very different sentiment, both positive and negative, which makes sense.  
